...
void ExampleScript::Test()
{
	...
	{
		owner->transform.SetGlobalMatrixTranslation(float3(transform.x + speed, transform.y, transform.z));
	}
	if (App->input->GetKey(KeyCode::W))
	{
		owner->transform.SetGlobalMatrixTranslation(float3(transform.x, transform.y, transform.z + speed));
	}
