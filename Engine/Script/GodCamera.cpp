...
void GodCamera::Update()
{
	...
	{
		owner->transform.SetGlobalMatrixTranslation(game_camera->transform.GetGlobalTranslation());
		owner->transform.SetRotation(game_camera->transform.GetRotation());
		god_mode = !god_mode;
		Active();
	}
	if (App->input->GetKeyDown(KeyCode::Alpha1) || input_manager->GetControllerButtonDown(ControllerCode::Back, ControllerID::ONE))
	{
		if (debug_mode->debug_enabled || god_mode)
		{
			owner->transform.SetGlobalMatrixTranslation(game_camera->transform.GetGlobalTranslation());
...
void GodCamera::GodModeCamera()
{
	...
	if (App->input->GetKey(KeyCode::E) || input_manager->GetControllerButton(ControllerCode::DownDpad, ControllerID::ONE))
	{
		camera_component->MoveDown();
	}
	if (App->input->GetKey(KeyCode::Q) || input_manager->GetControllerButton(ControllerCode::UpDpad, ControllerID::ONE))
	{
	...
	{
		float3 direction = right_axis_direction * rotation_speed + owner->transform.GetGlobalTranslation();;
	...
	{
		float3 direction = left_axis_direction * rotation_speed + owner->transform.GetGlobalTranslation();
